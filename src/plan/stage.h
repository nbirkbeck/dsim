...
struct Stage {
  ...
  struct Segment {
    ...
    Segment(RoadSegment* s = nullptr, int si = 0, int ei = 0)
        : road_segment(s), start_index(si), end_index(ei) {
      if (s) {
        const int len =
            (end_index - start_index + 2 * (int)road_segment->points.size()) %
            (int)road_segment->points.size();
        info.resize(len + 1);
      }
    }
    double GetTotalLength(int index) const {
      return (info[index].smooth_end
                  ? info[index].segment_length +
                        (info[index].curve_length - kRoundingDist)
                  : info[index].segment_length);
    }

    void GetPoints(int index, nacb::Vec2f* p1, nacb::Vec2f* p2) const {
      const int i1 = (start_index + index) % (int)road_segment->points.size();
      const int i2 =
          (start_index + index + 1) % (int)road_segment->points.size();
    ...
    bool IsLastIndex(int sub_index) const {
      return end_index ==
             (start_index + sub_index) % (int)road_segment->points.size();
    }

    bool GetInterpolatedPoint(int index, double* distance_to_travel,
                              nacb::Vec2f* pos) {
      ...
      const double rounding_t =
          (*distance_to_travel - rounding_start) / (info[index].curve_length);
      if (info[index].smooth_start) {
        p1 = info[index].modified_start;
      }
      if (info[index].smooth_end && rounding_t >= 0) {
        *pos = QuadraticBezier(info[index].anchors[0], p2,
                               info[index].anchors[1], rounding_t);
      } else {
        *pos = Interpolate(p1, p2,
                           *distance_to_travel / info[index].segment_length);
      }
      return true;
    }
  };

  Stage(Type t = NONE, const std::vector<Segment>& segs = {})
      : type(t), segments(segs) {
    if (type == ROAD_TRAVEL)
      BuildCurveInfo();
  }
    ...
    for (int i = 0; i < (int)segments.size(); ++i) {
      for (int j = 0; !segments[i].IsLastIndex(j); ++j) {
        ...
        if (GetUpcomingPoint(i, j, &next_segment, &p3)) {
          ...
          if (d2_len >= 1 && d1_len >= 1 && d1.dot(d2) < kSmoothThreshold) {
            prev_smooth = true;
            ...
            segments[i].info[j].curve_length =
                EstimateCurveLength(segments[i].info[j].anchors[0], p2,
  ...
  bool GetUpcomingPoint(int segment_index, int sub_index,
                        Segment** next_segment, nacb::Vec2f* p3) {
    if (type != ROAD_TRAVEL)
      return false;
